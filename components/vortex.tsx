...
export const Vortex = (props: VortexProps) => {
  ...
  const baseHue = props.isDark ? 190 : 220; // Light blue for dark mode, blue for light mode
  const rangeHue = props.isDark ? 30 : 40;
  const noiseSteps = 3;
  const xOff = 0.00125;
  const yOff = 0.00125;
  const zOff = 0.0005;
  ...
  const drawParticle = (
    ...
    const saturation = props.isDark ? '85%' : '70%';
    const lightness = props.isDark ? '80%' : '60%';
    ctx.strokeStyle = `hsla(${hue},${saturation},${lightness},${fadeInOut(life, ttl) * 0.8})`;
    ctx.beginPath();
    ctx.moveTo(x, y);
    ctx.lineTo(x2, y2);
    ctx.stroke();
